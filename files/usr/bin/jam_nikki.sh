...
function nyetop() {
	...
	if [[ -f "$initd"/nikki ]] && [[ $(uci -q get nikki.config.enable) == "1" ]]; then "$initd"/nikki stop && echo -e "${stopvpn} nikki"; fi
}
...
function nyetart() {
	...
	if [[ -f "$initd"/nikki ]] && [[ $(uci -q get nikki.config.enable) == "1" ]]; then "$initd"/nikki restart && echo -e "${startvpn} nikki"; fi
	fi
}
...
# Extended GMT+7 by vitoharhari
# Simplify usage and improved codes by helmiau
# Supported VPN Tunnels: nikki, Passwall, ShadowsocksR, ShadowsocksR++, v2ray, v2rayA, xray, Libernet, Xderm Mini, Wegare
